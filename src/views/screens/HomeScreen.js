      ...
      {categories.map((item, index) => (
        <TouchableOpacity key={index} activeOpacity={0.8} onPress={() => setSelectedCategoryIndex(index)}>
          <View>
            ...
            {selectedCategoryIndex == index && (
              <View style={{ height: 3, widht: 30, backgroundColor: COLORS.primary, marginTop: 2 }} />
            )}
          </View>
        </TouchableOpacity>
      ))}
      ...
      <TouchableOpacity disabled={activeCardIndex != index} activeOpacity={1} onPress={()=>navigation.navigate("DetailsScreen", hotel)} >
          <Animated.View style={{ ...style.card, transform: [{ scale }] }}>
            ...
            <View style={style.priceTag}>
              <Text style={{ color: COLORS.white, fontSize: 20, fontWeight: 'blod' }}>${hotel.price}</Text>
            </View>
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.white }}>
      <View style={style.header}>
        <View style={{ paddingBottom: 15 }}>
          ...
          <View style={{ flexDirection: 'row' }}>
            ...
            <Text style={{ fontSize: 30, fontWeight: 'bold', color: COLORS.primary }}> rabat </Text>
          </View>
        </View>
        <Icon name="person-outline" size={38} color={COLORS.gray} />
      </View>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginHorizontal: 20 }}>
          <Text style={{ fontWeight: 'blod', color: COLORS.grey }}>Top hotels</Text>
